...
def processing(username='train'):
    ...
    np.savez_compressed(os.path.sep.join([config.PROCESS_DATA_DIR, username,
...
def training(username='train', new=False, label=''):
    """ loading embedding data for training"""
    data = np.load(os.path.join(config.PROCESS_DATA_DIR, username, 'embedding_face.npz'))
    Xtrain, ytrain = data['arr_0'], data['arr_1']
    ...
    path_model = os.path.join(config.MODEL_DIR, username)
...
def evaluation():
    ...
    model = pickle.load(open(os.path.sep.join([config.APP, config.MODEL_NET, config.MODEL_DIR, 'model.pickle']), 'rb'))
    processed_data = os.path.sep.join([config.APP, config.MODEL_NET, config.PROCESS_DATA])
